# Implementation Roadmap v3 — от и до

**Дата:** 2026-04-08  
**Ключевое изменение vs v2:** переиспользуем code из POS/coliving-brain, ingest через free OpenRouter models
...
| MCP server | FastMCP Python (~50 строк) поверх search | без LLM |
...
**Объём:** ~150 строк нового кода + конфиг файлы
...
**Файл `/home/dev/POS/wiki/scripts/ingest.py`** — адаптация `phase2_llm.py`:
...
def update_catalog(topic: str, vault: str, record_type: str, path: str):
    ...
    # Remove old entry for same topic+type if exists
...
**Что делаем:** адаптируем готовый BM25-like scorer из coliving под wiki catalog.

**Файл `/home/dev/POS/wiki/scripts/search.py`** — адаптация `search_content.py`:

```python
#!/usr/bin/env python3
"""Wiki search: BM25-like over record-catalog + SUMMARY files."""
from __future__ import annotations
import json, re, sys
from pathlib import Path
...
def score(entry: dict, terms: list[str]) -> int:
    ...
    score = 0
    for term in terms:
        score += topic.count(term) * 8   # title-weighted
...
- [ ] результат содержит SUMMARY текст, не путь к файлу
...
```python
#!/usr/bin/env python3
"""Wiki lint: deterministic quality checks."""
from __future__ import annotations
import json, re
from pathlib import Path
...
**Файл `/home/dev/POS/wiki/scripts/wiki_mcp.py`** — FastMCP поверх search.py:

```python
#!/usr/bin/env python3
"""Wiki MCP server — 3 tools for Layer 0."""
import json
from pathlib import Path
...
def wiki_search(query: str, vault: str = "pos") -> str:
    """Search wiki records by query. Returns top-5 SUMMARY records."""
...
- [ ] Claude видит инструменты `wiki_search`, `wiki_get`, `wiki_list_topics`
...
- [ ] `python3 scripts/lint.py` → LINT OK
- [ ] `python3 scripts/search.py "codex"` → результат без открытия raw
- [ ] MCP tools видны в Claude
...
- `ingest.py` получает delta mode: читает `ingest-state.json`, пропускает уже обработанные
- `capture.sh` расширяется: auto-detect новых Issues через `gh issue list --since`
- COLLECTION records: timeline для decisions
- Cron или hook при новом session snapshot

**Ключевые изменения кода:**
- Добавить checksum в `ingest-state.json`
- `ingest.py --delta` — только новые/изменённые
- Write-back: после query Claude пишет в `query-writeback.md`, раз в неделю review

---

## Layer 2 — Coliving vault (после разморозки coliving-brain)

- `raw/coliving/` — адаптер из `notion_coliving_v4.py` (уже умеет читать Notion)
- `wiki/coliving/` — namespaces: concepts, operations, initiatives, market, assets
- Тот же `ingest.py` с параметром `--vault coliving`
- `wiki_search(query, vault="coliving")` — уже работает

---

## Layer 3 — Оптимизация (когда Layer 1 стабилен)

- Stable ingest prompts → prompt caching 50-90%
- Batch compilation (≥3 sources за вызов)
- SQLite FTS5 вместо jsonl scan (адаптация из `search_content.py` + SQLite)
...
- Diff перед записью wiki изменений
...
0.4 Search script            (~10 мин, в основном адаптация)
0.5 Lint                     (~10 мин)
0.6 MCP server               (~15 мин)
────────────────────────────
Итого:                       ~80 мин
```
